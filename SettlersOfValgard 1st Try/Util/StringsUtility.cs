...
namespace SettlersOfValgard.Util
{
    ...
    {
        public static string InsertCommas(this List<string> items)
        {
        ...
        public static List<T> Match<T>(string input, IEnumerable<T> list, string suffix = null, string contains = null) where T : INamed
        {
        ...
        public static List<T> Match<T>(this IEnumerable<T> list, string input, string suffix = null, string contains = null) where T : INamed
        {
            var ans = list.Where(item => item.Name.ToLower().StartsWith(input.ToLower())).ToList();
            if (suffix != null) ans = ans.Where(item => item.Name.ToLower().EndsWith(suffix.ToLower())).ToList();
            if (contains != null) ans = ans.Where(item => item.Name.ToLower().Contains(contains.ToLower())).ToList();
