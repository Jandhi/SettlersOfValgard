...
namespace SettlersOfValgard.Model.Resource
{
    public class PositiveResourceLedger : ResourceLedger
    {
        ...
        public PositiveResourceLedger Where(Func<KeyValuePair<Resource, int>, bool> filter)
        {
            var filteredContents = Contents.Where(filter).ToDictionary(pair => pair.Key, pair => pair.Value);
            return new PositiveResourceLedger(filteredContents);
        }
